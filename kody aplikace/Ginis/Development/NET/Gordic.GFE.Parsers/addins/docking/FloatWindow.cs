...
//    <Name>        Gordic.GFE.Parsers.FloatWindow.cs                         </Name>
//    <Description> Plovoucí okno                                               </Description>
...
namespace Gordic.GFE.Parsers.Docking
{
	...
	public class FloatWindow : Form, INestedPanesContainer, IDockDragSource
	{
        ...
        /// <param name="bounds">Rámec zobrazení</param>
        ...
        {
            if (dockPanel == null)
                throw (new ArgumentNullException(GResources.GetResourceText(29450063) + '\n' + GResources.GetResourceText(29450084))); //RC 29450084 : argument podokna nemùže být NULL!
        ...
        internal void SetText()
		{
			...
			if (theOnlyPane == null)
				Text = " ";	// schválnì " " místo string.Empty prootže jinak celý titulek zmizí
		...
		protected override void WndProc(ref Message m)
		{
			if (m.Msg == (int)Msgs.WM_NCLBUTTONDOWN)
			{
                ...
                IntPtr result = new IntPtr(NativeMethods.SendMessage(Handle, (int)Msgs.WM_NCHITTEST, IntPtr.Zero, m.LParam));
				if (result.ToInt32() == 2 && DockPanel.AllowEndUserDocking && this.AllowEndUserDocking)	// HITTEST_CAPTION
				{
					Activate();
            ...
            {
                IntPtr result = new IntPtr(NativeMethods.SendMessage(Handle, (int)Msgs.WM_NCHITTEST, IntPtr.Zero, m.LParam));
                if (result.ToInt32() == 2)	// HITTEST_CAPTION
                {
                    DockPane theOnlyPane = (VisibleNestedPanes.Count == 1) ? VisibleNestedPanes[0] : null;
		...
		internal void TestDrop(IDockDragSource dragSource, DockOutlineBase dockOutline)
		{
            if (VisibleNestedPanes.Count == 1)
            {
                ...
                IntPtr lParam = Win32.MakeLong(ptMouse.X, ptMouse.Y);
                if (NativeMethods.SendMessage(Handle, (int)Msgs.WM_NCHITTEST, IntPtr.Zero, lParam) == (int)HitTest.HTCAPTION)
