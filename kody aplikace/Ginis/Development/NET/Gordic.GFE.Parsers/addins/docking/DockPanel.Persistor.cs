...
//    <Name>        Gordic.GFE.Parsers.DockPanel.Persistor.cs                 </Name>
//    <Description> Dokovatelný panel                                           </Description>
    ...
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            public static void SaveAsXml(DockPanel dockPanel, string fileName, Encoding encoding)
            {
                ...
                try { SaveAsXml(dockPanel, fs, encoding); }
                finally { fs.Close(); }
            }
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                {
                    // použijeme odsazení pro èítelnost
                ...
                xmlOut.WriteComment(GResources.GetResourceText(29450079)); //RC 29450079 : Konfiguraèní soubor podokna.
                xmlOut.WriteComment(GResources.GetResourceText(29450080)); //RC 29450080 : !!! AUTOMATICKY generovaný soubor. Neupravujte !!!
            ...
            public static void LoadFromXml(DockPanel dockPanel, string fileName, DeserializeDockContent deserializeContent)
            {
                ...
                try { LoadFromXml(dockPanel, fs, deserializeContent); }
                finally { fs.Close(); }
            }
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    if (xmlIn.Name != "Content" || id != i)
                        throw new ArgumentException(GResources.GetResourceText(29450081)); //RC 29450081 : XML formát souboru je neplatný!
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                ...
                if (dockPanel.Contents.Count != 0)
                    throw new InvalidOperationException(GResources.GetResourceText(29450082)); //RC 29450082 : Podokno již bylo inicializováno!
                ...
                if (!IsFormatVersionValid(formatVersion))
                    throw new ArgumentException(GResources.GetResourceText(29450083)); //RC 29450083 : Verze konfiguraèního souboru je neplatná!
                ...
                // nastavení Zindexu dokovaného okna
                ...
                // pøiøazení podokna dokovatelným oknùm
                for (int i = 0; i < dockWindows.Length; i++)
                ...
                // øazení IDockContent dle Zindexù podoken
                ...
                // zobrazení ne dokumentù IDockContent jako prvních, aby zabranit blikání obrazovky
                for (int i = 0; i < contents.Length; i++)
                {
                ...
                // teï všechny dokumenty IDockContent
                for (int i = 0; i < contents.Length; i++)
                {
                    IDockContent content = dockPanel.Contents[sortedContents[i]];
                    if (content.DockHandler.Pane != null && content.DockHandler.Pane.DockState == DockState.Document)
        ...
        /// Uložení jako XML
        /// </summary>
        /// <param name="fileName">Název souboru XML</param>
        public void SaveAsXml(string fileName)
        {
            Persistor.SaveAsXml(this, fileName);
        }
        /// <summary>
        /// Uložení jako XML do souboru
        /// </summary>
