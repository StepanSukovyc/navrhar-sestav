...
//    <Name>        Gordic.GFE.WinClient.RdiDesktopLayout.cs                  </Name>
...
namespace Gordic.GFE.WinClient.Base.Gui
{
    ...
    /// pomocný panel
    /// </summary>
    sealed class CustomToolStripPanel : ToolStripPanel
    {
        bool isMoved = false;
        /// <summary>
        /// aktualizace panelů
        /// </summary>
        internal void _Refresh()
        {
            if (!isMoved)
            {
    ...
    /// </summary>
    sealed class RdiDesktopLayout : IDesktopLayout, IDisposable
    {
        ...
        public void Attach(IDesktop desktop)
        {
            ...
            toolBarPanel = new CustomToolStripPanel();
        ...
        {
            foreach (RdiWorkspaceWindow f in SimpleDesktop.Desktop.DesktopWindowCollection)
                try
                {
                    f.DockPanel = null;
                    if (dispose)
        ...
        {
            RdiWorkspaceWindow f = (RdiWorkspaceWindow)sender;
            f.CloseEvent -= CloseWindowEvent;
        ...
        void LoadLayoutConfiguration()
        {
            ...
            catch (Exception ex)
            {
                ...
                LoggingService.Info(GResources.GetResourceText(29450318)); //RC 29450318 : Ignorujeme chyby načítání konfigurace rozložení
            }
        }
        void LoadDefaultLayoutConfiguration()
        {
            if (!File.Exists(LayoutConfiguration.CurrentLayoutTemplateFileName))
                ResourceService.SaveFile(typeof(ReportDesignerMain).Assembly, Path.Combine(PropertyService.DataDirectory, LayoutConfiguration.DataLayoutSubPath), @"^*layouts.Default.xml", "layouts.");
            if (File.Exists(LayoutConfiguration.CurrentLayoutTemplateFileName))
        ...
        void ActiveContentChanged(object sender, EventArgs e) { OnActiveDesktopWindowChanged(e); }
        ...
        {
            if (content.DesktopWindow is RdiWorkspaceWindow oldSdiWindow)
            {
            ...
            RdiWorkspaceWindow rdiWorkspaceWindow = new RdiWorkspaceWindow();

            rdiWorkspaceWindow.ViewContents.Add(content);
            if (content.SecondaryViewContents != null)
                rdiWorkspaceWindow.ViewContents.AddRange(content.SecondaryViewContents);
            rdiWorkspaceWindow.SwitchView(ReportDesignerProperties.Instance.TabDefaultViewIndex);
        ...
        void AddWindowToDockPanelWithoutSwitching(RdiWorkspaceWindow rdiWorkspaceWindow)
        {
            rdiWorkspaceWindow.DockPanel = dockPanel;
            if (!(dockPanel.ActiveContent is RdiWorkspaceWindow otherWindow))
                otherWindow = dockPanel.Contents.OfType<RdiWorkspaceWindow>().FirstOrDefault(c => c.Pane != null);
            if (otherWindow != null)
        ...
        public void ActivatePad(string fullyQualifiedTypeName)
        {
            try
            {
                if (contentHash.TryGetValue(fullyQualifiedTypeName, out PadContentWrapper value))
                    value.Show();
            }
            catch { }
        }
        ...
        void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                if (statusStripContainer != null)
                {
                    statusStripContainer.Dispose();
                    statusStripContainer = null;
                }
        ...
        /// uvolnění objektu
        /// </summary>
        ~RdiDesktopLayout() { Dispose(false); }
        #endregion

        DefaultDesktop wbForm;
        DockPanel dockPanel;
        readonly Dictionary<string, PadContentWrapper> contentHash = new Dictionary<string, PadContentWrapper>();

        AutoHideMenuStripContainer mainMenuContainer;
        AutoHideStatusStripContainer statusStripContainer;
        CustomToolStripPanel toolBarPanel;
