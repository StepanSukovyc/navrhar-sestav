...
//    <Name>          Gordic.General.GDateConverter.cs           </Name>
//    <Description>   typový konvertor pro databázový typ GDate  </Description>
...
namespace Gordic.General {
	...
	public class GDateConverter : TypeConverter {
		...
		public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object sourceValue) {
            if (sourceValue is string && sourceValue!=null) return GDate.Parse((String) sourceValue,culture);
            else if(sourceValue is GString && ((GString) sourceValue).IsNull==false) return GDate.Parse(sourceValue.ToString(),culture);
			else return GDate.Parse(sourceValue,true);
		} // end method
		...
		public override object ConvertTo(ITypeDescriptorContext context,CultureInfo culture, object sourceValue, Type destinationType) {  
			if(destinationType == null) throw new GArgumentNullException(23200094); // nebyla nastavena hodnota oèekávaného parametru
            GDate l_oSourceValue = sourceValue as GDate;
            if(l_oSourceValue != null) {
                try {
                    ...
                    else if(destinationType == typeof(GDate)) return l_oSourceValue.Clone();
                    ...
                    if(e is GException == false) throw new GOverflowException(23200104,e); // velikost hodnoty pøesáhla limit použitého datového typu
                    else throw;
                } // end catch
            } // end if
            throw new GInvalidCastException(23200095); // pokus o nepovolenou typovou konverzi
		} // end method
