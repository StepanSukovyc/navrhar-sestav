...
namespace Gordic.General {
	...
	/// <summary>typový konvertor pro typ GSecureString</summary>
	public class GSecureStringConverter : TypeConverter {
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object sourceValue) {
            return GSecureString.Parse(sourceValue);
        } // end method
        ...
        public override object ConvertTo(ITypeDescriptorContext context,CultureInfo culture, object sourceValue, Type destinationType) {  
            if(destinationType == null) 
                throw new GArgumentNullException(21300103); // nebyla nastavena hodnota očekávaného parametru
            GSecureString tajemstvi = sourceValue as GSecureString;
            if (tajemstvi != null && tajemstvi.Exists ) {
                try {
                    GString l_oSourceValue = new GString(tajemstvi.Secret);
                    ...
                    else if (destinationType == typeof(GSecureString)) return GSecureString.Parse(l_oSourceValue);
                } // end try
                catch(Exception e) {
                    if(e is GException == false) throw new GOverflowException(21300101,e); // velikost hodnoty přesáhla limit použitého datového typu
                    else throw;
                } // end catch
            } // end if
            throw new GInvalidCastException(21300102); // pokus o nepovolenou typovou konverzi
        } // end method
