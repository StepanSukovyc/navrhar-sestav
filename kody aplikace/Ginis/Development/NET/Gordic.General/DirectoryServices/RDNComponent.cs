...
//    <Name>        Gordic.General.RDNComponent.cs                              </Name>
//    <Description> Encapsulates a component of an RDN.  Most RDNs are made up of a single component,</Description>
...
namespace Gordic.General.DirectoryServices
{
	...
	public class RDNComponent
	{
		...
		public static string EscapeValue(string s, DNEscapeChars escapeChars)
		{
			...
			for(int i = 0; i < s.Length; i++)
			{
				...
				{
					if ((escapeChars & DNEscapeChars.SpecialChars) != DNEscapeChars.None)
						ReturnValue.Append('\\');

					ReturnValue.Append(s[i]);
				}
				else if (s[i] < 32 && ((escapeChars & DNEscapeChars.ControlChars) != DNEscapeChars.None))
				{
					ReturnValue.AppendFormat("\\{0:X2}", (int)s[i]);
				}
				else if (s[i] >= 128 && ((escapeChars & DNEscapeChars.MultibyteChars) != DNEscapeChars.None))
				{
