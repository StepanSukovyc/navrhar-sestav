...
using Gordic.TextEditor.SearchAndReplace;

namespace Gordic.TextEditor.Actions
{
...
//				document.Replace(line.Offset,line.Length,newLine);
		...
		/// </remarks>
		/// <param name="textArea">The which is used for callback purposes</param>
...
//						textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.PositionToLineEnd, new TextLocation(textArea.Caret.Offset - textArea.Document.GetLineSegment(curLineNr).Offset, curLineNr)));
    ...
    public class FindAction : AbstractEditAction
    {
        ...
        /// </summary>
        /// <param name="textArea">oblast, se kteru pracuje příkaz</param>
        public override void Execute(TextArea textArea)
        {
            if (textArea != null)
                Find.FindAction(textArea.MotherTextEditorControl);
        }
    }

    /// <summary>
    /// Najít další (i jako F3)
    /// </summary>
    public class FindNextAction : AbstractEditAction
    {
        /// <summary>
        /// metoda provedení příkazu
        /// </summary>
        ...
        {
            if (textArea != null)
                FindNext.FindNextAction(textArea.MotherTextEditorControl);
        }
    }
    ...
    public class ReplaceAction : AbstractEditAction
    {
        ...
        {
            if (textArea != null)
                Replace.ReplaceAction(textArea.MotherTextEditorControl);
        }
    }
}
