...
using Gordic.TextEditor.Misc.XmlForms;

namespace Gordic.TextEditor.SearchAndReplace
{
    /// <summary>
    /// panel hledání/nahrazení textu
    /// </summary>
	public class SearchAndReplacePanel : BaseSharpDevelopUserControl
	{
        ...
        /// </summary>
        bool HasFindPattern { get { return Get<ComboBox>("find").Text.Length != 0; } }
        bool IsSelectionSearch { get { return DocumentIteratorType == DocumentIteratorType.CurrentSelection; } }

        /// <summary>
        /// režim hledání/nahrazení
        /// </summary>
        public SearchAndReplaceMode SearchAndReplaceMode
        {
            get { return searchAndReplaceMode; }
            set
            {
                ...
                switch (searchAndReplaceMode)
                {
                    case SearchAndReplaceMode.Search:
                        SetupFromXmlStream(this.GetType().Assembly.GetManifestResourceStream("Gordic.TextEditor.Resources.Misc.SearchAndReplace.FindPanel.xfrm"));
                    ...
                    case SearchAndReplaceMode.Replace:
                        SetupFromXmlStream(this.GetType().Assembly.GetManifestResourceStream("Gordic.TextEditor.Resources.Misc.SearchAndReplace.ReplacePanel.xfrm"));
        ...
        {
            return IsTextSelected(selection) ? selection.SelectedText.IndexOf('\n') != -1 : false;
        }
        static bool IsTextSelected(ISelection selection)
        {
            return selection != null ? !selection.IsEmpty : false;
        }
        ...
        /// <summary>
        /// typ iterace - dle tohoto typu se hledá v různých oknách
        /// </summary>
        public DocumentIteratorType DocumentIteratorType
        {
            get { return (DocumentIteratorType)(Get<ComboBox>("lookIn").SelectedIndex); }
            set { Get<ComboBox>("lookIn").SelectedIndex = (int)value; }
        }
        ...
        void FindNextButtonClicked(object sender, EventArgs e)
        {
            ...
            if (IsSelectionSearch)
            {
                ...
                using (AsynchronousWaitDialog monitor = AsynchronousWaitDialog.ShowWaitDialog("Hledání", true))
        ...
        void FindAllButtonClicked(object sender, EventArgs e)
        {
            ...
            if (IsSelectionSearch)
            {
                ...
                using (AsynchronousWaitDialog monitor = AsynchronousWaitDialog.ShowWaitDialog("Hledání", true))
                    SearchReplaceManager.FindAll(monitor);
        }
        ...
        void SetOptions()
        {
            ...
            //Get<ComboBox>("find").AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            //Get<ComboBox>("find").AutoCompleteSource = AutoCompleteSource.ListItems;
            Get<ComboBox>("find").Items.Clear();
            foreach (string findPattern in SearchOptions.FindPatterns)
                Get<ComboBox>("find").Items.Add(findPattern);
            ...
            if (searchAndReplaceMode == SearchAndReplaceMode.Replace)
            {
                ...
                //Get<ComboBox>("replace").AutoCompleteMode = AutoCompleteMode.SuggestAppend;
                //Get<ComboBox>("replace").AutoCompleteSource = AutoCompleteSource.ListItems;
                Get<ComboBox>("replace").Items.Clear();
                foreach (string replacePattern in SearchOptions.ReplacePatterns)
                    Get<ComboBox>("replace").Items.Add(replacePattern);
            }
            ...
            foreach (string lookInText in lookInTexts)
                Get<ComboBox>("lookIn").Items.Add(lookInText);
            ...
            Get<ComboBox>("use").Items.Add("standardní vyhledávání");
            Get<ComboBox>("use").Items.Add("regulární výrazy");
            Get<ComboBox>("use").Items.Add("zástupné znaky");
        ...
        void FindNextInSelection()
        {
            ...
            finally { ignoreSelectionChanges = false; }
        }		
