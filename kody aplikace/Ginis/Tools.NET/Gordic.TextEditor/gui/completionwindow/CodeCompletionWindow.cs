...
namespace Gordic.TextEditor.Gui.CompletionWindow
{
	public class CodeCompletionWindow : AbstractCompletionWindow
	{
        ...
        {
            return completionDataProvider == null
                ? null
                : ShowCompletionWindow(parent
                    , control
                    , fileName
                    , firstChar
                    , completionDataProvider
                    , completionDataProvider.Options != null && completionDataProvider.Options.ShowDescriptionWindow
                    , true
                    , completionDataProvider.Options != null && completionDataProvider.Options.AutomateCompleteIfUnique
                    , completionDataProvider.GenerateCompletionData(fileName, control.ActiveTextAreaControl.TextArea, firstChar));
        }
        ...
        {
            return ShowCompletionWindow(parent
                , control
                , fileName
                , firstChar
                , completionDataProvider
                , showDeclarationWindow
                , fixedListViewWidth
                , completionDataProvider.Options != null && completionDataProvider.Options.AutomateCompleteIfUnique
                , completionDataProvider.GenerateCompletionData(fileName, control.ActiveTextAreaControl.TextArea, firstChar));
        }

        static CodeCompletionWindow ShowCompletionWindow(Form parent, TextEditorControl control, string fileName, char firstChar, ICompletionDataProvider completionDataProvider, bool showDescriptionWindow, bool fixedListViewWidth, bool completeIfUnique, ICompletionData[] completionData)
        {
            if (completionData == null || completionData.Length == 0)
                return null;

            CodeCompletionWindow codeCompletionWindow = new CodeCompletionWindow(completionDataProvider, completionData, parent, control, showDescriptionWindow, fixedListViewWidth);
            var data = codeCompletionWindow.codeCompletionListView.SelectedCompletionData;
            if (completeIfUnique && data != null && data.IsUnique)
            {
                control.ActiveTextAreaControl.TextArea.SelectionManager.ClearSelection();
                codeCompletionWindow.InsertSelectedItem('\n');
                codeCompletionWindow.Close();
                return null;
            }
            else
            {
                codeCompletionWindow.CloseWhenCaretAtBeginning = !string.IsNullOrEmpty(completionDataProvider.PreSelection) && firstChar == '\0';
        ...
        /// <summary>
        /// reakce na změnu umístění kurzoru
        /// </summary>
