	...
	internal sealed class DefaultDocument : IDocument
	{
		...
		public string TextContent {
			...
			set {
                ...
                OnDocumentAboutToBeChanged(new DocumentEventArgs(this, 0, textBufferStrategy.Length, value));
				textBufferStrategy.SetContent(value);
				lineTrackingStrategy.SetContent(value);
				undoStack.ClearAll();

                OnDocumentChanged(new DocumentEventArgs(this, 0, textBufferStrategy.Length, value));
				OnTextContentChanged(EventArgs.Empty);
			}
		}
		
        /// <summary>
        /// vložení textového řetězce <paramref name="text"/> na pozici <paramref name="offset"/>
        /// </summary>
        /// <param name="offset">pozice pro vložení textu <paramref name="text"/></param>
        /// <param name="text">vkládaný řetězec</param>
		public void Insert(int offset, string text)
		{
			if (readOnly)
				return;

            using (var arg = new DocumentEventArgs(this, offset, -1, text))
            {
                OnDocumentAboutToBeChanged(arg);

                textBufferStrategy.Insert(offset, text);
                lineTrackingStrategy.Insert(offset, text);
                undoStack.Push(new UndoableInsert(this, offset, text));

                OnDocumentChanged(arg);
            } 
		}
